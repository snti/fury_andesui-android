...
class WalkthroughMessageView @JvmOverloads constructor(
        ...
        when (position) {
            WalkthroughMessagePosition.BELOW -> {
                ...
                if (isNecessaryShowArrow(centerTooltip, centerTarget)) {
                    arcArrowTop.addRect(centerTooltip, tooltipRect.bottom - PADDING_TOP, centerTarget, tooltipRect.top - PADDING_TOP)
                } else {
            ...
            WalkthroughMessagePosition.ABOVE -> {
                ...
                centerTooltip = (tooltipRect.left + tooltipRect.right) / 2
                if (isNecessaryShowArrow(centerTooltip, centerTarget)) {
                    arcArrowBottom.addRect(centerTooltip, tooltipRect.top - PADDING_TOP, centerTarget, tooltipRect.bottom - PADDING_TOP)
                } else {
                    arcArrowBottom.visibility = View.GONE
                }
            }
    ...
    private fun isNecessaryShowArrow(centerTooltip: Int, centerTarget: Int): Boolean {
        if (centerTooltip - centerTarget >= 0 && centerTooltip - centerTarget <= MIN_WIDTH_FOR_SHOW_ARROW) {
            return false
        } else if (centerTarget - centerTooltip >= 0 && centerTarget - centerTooltip <= MIN_WIDTH_FOR_SHOW_ARROW) {
            return false
        }
        return true
    }
    ...
    companion object {
        val MIN_WIDTH_FOR_SHOW_ARROW = ViewUtils.dpToPx(48)
        val PADDING_TOP = ViewUtils.dpToPx(16)
    }
}